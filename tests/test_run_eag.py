...
def test_add_series():
    e = test_make_eag()
    e.add_eag_series(pd.Series(index=pd.date_range("2000", periods=10, freq="D"), data=1e-3*np.ones(10)), name="Neerslag", tmin="2000", tmax="2000-01-10", 
                     fillna=True, method=0.0)
    e.add_eag_series(pd.Series(index=pd.date_range("2000", periods=10, freq="D"), data=0.25e-3*np.ones(10)), name="Verdamping", tmin="2000", tmax="2000-01-10", 
                     fillna=True, method=0.0)
    return e
...
def test_add_real_series_and_simulate():
    ...
    # add external series
    series = pd.read_csv(os.path.join(test_data, "series_1396_3360-EAG-1.csv"), sep=";", parse_dates=True, index_col=[1])
    
    # Loop over Gemaal, Inlaat, Uitlaat:
    factor = 1.0
    for seriestype in ["Gemaal", "Inlaat", "Uitlaat"]:
        colmask = [True if icol.startswith(seriestype) else False for icol in series.columns]
        if np.sum(colmask) == 0:
            continue
        selected_series = series.loc[:, colmask]
        if seriestype == "Gemaal":
            selected_series = selected_series.sum(axis=1)
            e.add_eag_series(selected_series, name=seriestype, tmin=tmin, tmax=tmax, 
                             fillna=True, method=0.0)
        else:
            if seriestype == "Uitlaat":
                factor = -1.0
            for jcol in selected_series.columns:
                e.add_eag_series(factor*selected_series.loc[:, jcol], name=jcol.split("|")[0], 
                                 tmin=tmin, tmax=tmax, fillna=True, method=0.0)

    # Peil
    colmask = [True if icol.lower().startswith("peil") else False for icol in series.columns]
    ...
    colmask = [True if icol.lower().startswith("neerslag") else False for icol in series.columns]
    e.series.loc[intersection, "Neerslag"] = series.loc[intersection, colmask].fillna(0.0).values.squeeze() * 1e-3
    colmask = [True if icol.lower().startswith("verdamping") else False for icol in series.columns]
    e.series.loc[intersection, "Verdamping"] = series.loc[intersection, colmask].fillna(0.0).values.squeeze() * 1e-3
