...
def test_make_eag():
    ...
    eag_id = 1396
    buckets = pd.read_csv(
        os.path.join(test_data, "opp_1396_3360-EAG-1.csv"), delimiter=";", decimal="."
    )
    buckets["OppWaarde"] = pd.to_numeric(buckets.OppWaarde)
    e = wb.create_eag(
        eag_id, name, buckets, use_waterlevel_series=False, log_level=logging.DEBUG
    )
    return e
...
def test_add_series_to_eag():
    ...
    e.add_timeseries(
        pd.Series(
            index=pd.date_range("2000", periods=10, freq="D"), data=1e-3 * np.ones(10)
        ),
        name="Neerslag",
        tmin="2000",
        tmax="2000-01-10",
...
def test_add_real_series_and_simulate():
    ...
    reeksen = pd.read_csv(
        os.path.join(test_data, "reeks_1396_3360-EAG-1.csv"), delimiter=";", decimal=","
    )
    # get EAG
    e = test_make_eag()
    ...
    # add external series
    series = pd.read_csv(
        os.path.join(test_data, "series_1396_3360-EAG-1.csv"),
        sep=";",
        parse_dates=True,
        index_col=[1],
    )
    ...
    for seriestype in ["Gemaal", "Inlaat", "Uitlaat"]:
        colmask = [
            True if icol.startswith(seriestype) else False for icol in series.columns
        ]
        if np.sum(colmask) == 0:
            continue
        ...
        if seriestype == "Gemaal":
            ...
            for jcol in selected_series.columns:
                e.add_timeseries(
                    factor * selected_series.loc[:, jcol],
                    name=jcol.split("|")[0],
                    tmin=tmin,
                    tmax=tmax,
                    fillna=True,
    ...
    # Peil
    colmask = [
        True if icol.lower().startswith("peil") else False for icol in series.columns
    ]
    ...
    colmask = [
        True if icol.lower().startswith("neerslag") else False
        for icol in series.columns
    ]
    e.series.loc[intersection, "Neerslag"] = (
        series.loc[intersection, colmask].fillna(0.0).values.squeeze() * 1e-3
    )
    colmask = [
        True if icol.lower().startswith("verdamping") else False
        for icol in series.columns
    ]
    e.series.loc[intersection, "Verdamping"] = (
        series.loc[intersection, colmask].fillna(0.0).values.squeeze() * 1e-3
    )
